        ...
        <div class="content-card-body" style="padding: 24px;">
            <form action="{{ route('roles.store') }}" method="POST">
                ...
                <div style="margin-bottom: 24px;">
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3" style="width: 100%; padding: 10px 14px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 14px; color: #1e293b; outline: none; transition: border-color 0.2s; resize: vertical;">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p style="color: #ef4444; font-size: 12px; margin-top: 4px;">{{ $message }}</p>
                    @enderror
                </div>
